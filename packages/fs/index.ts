...
export function write(h: number, str: string | Uint8Array) {
    //return do_syscall("write", h, str);
    /*if (str instanceof Uint8Array) {
        // sus workaround for bug (won't save uint8arrays)
        let nstr = "";
        for (let a of str) {
            nstr += String.fromCharCode(a);
        }
        str = nstr;
        console.log(str)
    }*/
...
export function stat(file: string): Promise<t_fstat> {
    //return do_syscall("fs_stat", file);
    return api.fs.stat(file);
}

export async function readFileBuffer(file: string): Promise<Uint8Array> {
